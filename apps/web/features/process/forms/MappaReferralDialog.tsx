...
import { mappaReferralFormSchema, type MappaProcess, type MappaReferralForm } from '@mas/domain';
...
export function MappaReferralDialog({ open, onClose, process }: { open: boolean; onClose: () => void; process: MappaProcess }) {
  ...
  const form = useForm<MappaReferralForm>({
    ...
    defaultValues: { category: d.category, levelSought: d.level === 3 ? 3 : 2, leadResponsibleAuthority: d.leadResponsibleAuthority, riskAssessmentIds: risks.map((r) => r.id), reason: '', imminentRisk: false, victimConsiderations: d.rmp?.victimSafety ?? '', accommodationIssue: Boolean(d.era), disclosureConsidered: d.disclosures.length > 0, visorReference: d.visorReference },
  });
