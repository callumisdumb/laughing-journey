...
|---|---|---|
| Two colluding escrow holders can read any record | There is no cryptographic answer to collusion at the threshold. Raising the threshold trades against availability. | Five holders in five organisations with different lines of accountability; every use signed by both and notified to the other three; a holder who is an excluded party on the record being opened is refused in code and the refusal is audited; a recovery does not restore a case the person is excluded from |
...
| In a browser the device private key is in localStorage, readable by any script running in the page origin | A browser has no equivalent of the OS keychain | The desktop shells are the supported deployment and hold the key in the keychain; the browser build is for development and demonstration, and the product says so on its Security page. A deployment that permits browser access should treat a cross-site scripting flaw in the application as a key compromise |
