...
## Re-run on 07 Sep 2026

Swept again after the round that emptied the handover's limits list. The sweep is the same: every button without a handler, every handler that only toasts, every link to a route that does not exist. It found nothing. The closing sentence above pointed readers at handover section 6 for what a practitioner cannot do, and that list is now empty, so the pointer is kept for the shape of the answer rather than its contents: an absent control is still not a dead one, and what remains absent are the limits of a mockup with no backend, which section 6 lists under "Product limits by design".

Three controls added this round say in the interface that they send nothing, because they name a recipient and a person could reasonably expect otherwise: the transfer to another authority, the information request to somebody outside the partnership, and the action given to an external owner. Each is a real record with real consequences inside the product; none of them is a control that appears to work and leaves no trace, which is what this audit exists to catch.
