...
Not in the catalogue: synthetic data (names, addresses, chronology facts, minutes text from the seed), identifiers and reference numbers, log lines, developer-facing console errors and code comments.

Nor is text that a module writes into a record or into configuration rather than rendering. The default need-to-know rows and exclusions, the classification markings, the forms and their versions in `default-config.ts`, the note a clock transition writes on a trigger, the case-role register entries generated from a referral, and the connector fixtures are all data: the first group is edited in Admin's own editors and stored in the config, the rest are stamped onto records at the time of an event and stay as they were written. Putting them in the catalogue would give the same string two editors and would rewrite history when someone changed a label. `pnpm messages:extract` skips those modules for that reason; everything a component renders is in scope. Dates, times and numbers are never written into messages; they are formatted with the `Intl` helpers in `@mas/messages` (`formatDate` gives dd Mon yyyy, `formatTime` the 24-hour clock, both in Europe/London) and passed into messages as arguments.
