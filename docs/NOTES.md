...
- Electron packaging re-run after the desktop namespace: `electron-builder --dir --linux` bundles `resources/en-GB.json` beside the export, and the compiled main process resolves "About Platform", "View", "Minimise" and the rest from it with the saved overrides file applied. The Tauri shell carries the same keys through `include_str!` but has not been compiled in this container (no GTK).
