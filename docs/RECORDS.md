...
| **Plan** | From a process at the right stage. | Outcomes, linked actions, review date. | **Ended**, superseded by a review, or the process closes. | Correction with reason. | Audit; clocks (plan review); chronology milestone | As its process |
...
**Merge.** Two person records become one. Every relationship, process membership, chronology event, sharing record and audit entry repoints to the survivor; the merged-away identifier is retained as a permanent alias so an inbound connector event carrying the old reference still lands. Requires a reason, is audited, notifies every case the two records touched, and writes to the survivor's chronology. **It is reversible**, because conflating two children is worse than the duplicate it was meant to fix and it does happen.
