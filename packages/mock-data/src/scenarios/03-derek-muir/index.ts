...
export function seedDerekMuir(ctx: BuildContext): void {
  ...
  const preCustody = makeAddress(ctx, { id: 'adr_derek_cannon_loan', line1: '17 Cannon Loan', line2: 'Flat 1/2', town: 'Auchentorran', postcode: 'QX2 3HW' });
  const abbeyWynd = makeAddress(ctx, { id: DEREK.currentAddress, line1: '40 Abbey Wynd', line2: 'Supported accommodation, room 4', town: 'Auchentorran', postcode: 'QX2 6TD' });
  const schoolhouseLoan = makeAddress(ctx, { id: DEREK.proposedAddress, line1: '6 Schoolhouse Loan', town: 'Craiglarrick', postcode: 'QX5 1AR' });
  ...
  const process: Process = {
    ...
    stage: 'managed',
    stageHistory: [
      { stage: 'notification', at: at('2026-05-10', '14:00'), byUserId: colin, byName: name(colin), note: 'Pre-release notification from HMP Dunlarrick, ten weeks before the release date' },
    ...
    detail: {
      ...
      victimPersonIds: [],
      notification: { at: at('2026-05-10', '14:00'), source: 'Scottish Prison Service, HMP Dunlarrick (pre-release notification)', byName: name(colin) },
      referral: {
      ...
      custody: { releasedAt: '2026-07-22', licenceExpiresAt: '2028-01-21', establishment: 'HMP Dunlarrick' },
      licenceConditions: [
        { id: 'lic_derek_1', text: 'To reside at 40 Abbey Wynd, Auchentorran and not to change address without the prior approval of the supervising officer', status: 'active' },
      ...
      rmp: {
        ...
        accommodation: 'Temporary supported accommodation at 40 Abbey Wynd, Auchentorran, with staff on site and no children resident. A move to a settled tenancy at 6 Schoolhouse Loan, Craiglarrick has been proposed by housing; an Environmental Risk Assessment is in progress and no decision will be made until it is complete.',
        employment: 'Derek wants to work. Any employment needs the supervising officer\'s approval and a disclosure decision where the role could bring contact with children or where the employer needs to know the licence conditions. A job offer from Auchentorran Vehicle Recycling Ltd is under consideration.',
        associates: 'No known associates of concern. Derek has no contact with his former partner. His brother in Dunlarrick is supportive and aware of the licence conditions.',
      ...
      era: {
        ...
        concerns: [
          ...
          'The bus stop for Auchentorran is outside the school gate, so the school run and Derek\'s likely travel times overlap',
        ],
      },
      disclosures: [
        {
          id: 'dsc_derek_1',
          recipient: 'Auchentorran Vehicle Recycling Ltd',
  ...
  makeMeeting(ctx, {
    ...
    location: 'Auchentorran Police Station, MAPPA room (restricted)',
    status: 'held',
    chairUserId: priya,
    ...
    preMeetingRequests: [
      ...
      { id: 'pmr_derek_pre_3', agency: 'health', toName: name(louise), toUserId: louise, sentAt: at('2026-06-16', '09:00'), dueAt: '2026-07-07', status: 'returned', returnSummary: 'No mental health service involvement. GP registration at Craiglarrick Health Centre to be arranged on release', returnedAt: at('2026-07-06', '16:20') },
      { id: 'pmr_derek_pre_4', agency: 'housing', toName: name(mark), toUserId: mark, sentAt: at('2026-06-16', '09:00'), dueAt: '2026-07-07', status: 'returned', returnSummary: 'Supported accommodation place identified at 40 Abbey Wynd, Auchentorran, outside the exclusion zone', returnedAt: at('2026-07-03', '10:30') },
  ...
  share('shr_derek_5', 'notification', ross, 'social-work', 'MAPPA Coordinator', 'full', 'Notification received. MAPPA Coordinator must know.', 'Pre-release notification from HMP Dunlarrick: Category 1, release 22 Jul 2026, ViSOR reference held', at('2026-05-11', '09:40'), priya, 'mappa.notification.coordinator');
  ...
  E({ occurredAt: at('2026-07-22', '09:30'), agency: 'sps', recordedByName: name(colin), recordedByUserId: colin, eventType: 'police.release', title: 'Released from HMP Dunlarrick on licence', detail: 'Released at the end of the custodial term. Licence to 21 Jan 2028. Met at the gate by the supervising officer and taken to 40 Abbey Wynd.', significance: 'high', significanceReason: 'Release into the community' });
  ...
  E({ occurredAt: at('2026-07-22', '12:30'), agency: 'housing', recordedByName: name(mark), recordedByUserId: mark, eventType: 'move.address', title: 'Moved to 40 Abbey Wynd, Auchentorran (supported accommodation)', detail: 'Placement started. Staff briefed on the residence condition and the contact for concerns.', significance: 'moderate' });
  E({ occurredAt: at('2026-07-23', '10:30'), agency: 'police', recordedByName: name(priya), recordedByUserId: priya, eventType: 'police.notification', title: 'Initial notification made under SONR at Auchentorran Police Station', detail: 'Attended in person within three days of release. Address, bank account and passport details recorded. Photograph taken. Next annual notification due 23 Jul 2027.', significance: 'moderate', significanceReason: 'SONR compliant' });
  ...
  makeAnalysis(ctx, {
    ...
    text: 'The flat is 180 metres from the school gate, faces a play park, and the Auchentorran bus stop is outside the school, so Derek\'s travel to the recycling yard would overlap with the school run twice a day. The licence condition on unsupervised contact with children does not stop incidental daily proximity. Controls that could be considered: a condition on the times he leaves and returns, a different bus route, or a different property. This note is an input to the ERA and the review meeting, not a decision. Housing should receive the ERA conclusions and controls only.',
  });
}
