...
export const WHINBRAE = {
  address: 'adr_whinbrae',
  household: 'hh_whinbrae',
  ...
  process: 'prc_asp_whinbrae',
  planningMeeting: 'mtg_whinbrae_lsi_planning',
  reviewMeeting: 'mtg_whinbrae_lsi_review',
  lawfulBasis: 'lb_whinbrae_asp',
} as const;

export function seedWhinbraeLsi(ctx: BuildContext): void {
  ...
  // ----- The setting -----
  const home = makeAddress(ctx, { id: WHINBRAE.address, line1: '1 Rowan Gait', line2: 'Whinbrae House', town: 'Craiglarrick', postcode: 'QX5 7LQ' });
  const lindaHome = makeAddress(ctx, { id: 'adr_linda_paterson', line1: '22 Loch Brae', town: 'Craiglarrick', postcode: 'QX5 4HN' });
  const robertHome = makeAddress(ctx, { id: 'adr_robert_kilgour', line1: '8 Seaforth Loan', town: 'Auchentorran', postcode: 'QX2 5PW' });
  ...
  const wilma = resident(WHINBRAE.wilma, 'Wilma', 'Sneddon', 'female', '1934-05-14', '2023-11-06', {
    communicationNeeds: { needs: ['Hard of hearing: sit on her left and speak clearly'] },
  });
  const jean = resident(WHINBRAE.jean, 'Jean', 'Kilgour', 'female', '1938-03-12', '2024-02-19', {
  ...
  const malcolm = resident(WHINBRAE.malcolm, 'Malcolm', 'Rankin', 'male', '1940-01-18', '2025-03-03', {
  ...
  const archie = resident(WHINBRAE.archie, 'Archie', 'Drummond', 'male', '1948-06-03', '2024-05-13', {
  ...
  const lb = makeLawfulBasis(ctx, {
    id: WHINBRAE.lawfulBasis,
    purpose: 'Large Scale Investigation under the Adult Support and Protection (Scotland) Act 2007 into medication errors and a financial irregularity at Whinbrae House',
  ...
  const process: Process = {
    ...
    title: 'Large Scale Investigation: Whinbrae House',
    subjectIds: all,
    ...
    members,
    clocks: [
      { id: 'clk_whinbrae_inquiry', ruleId: 'asp.inquiry.decision', triggeredAt: at('2026-07-30', '11:20'), completedAt: at('2026-08-06', '12:00'), note: 'Decision to proceed to investigation on working day 5' },
      { id: 'clk_whinbrae_conference', ruleId: 'asp.caseconference.initial', triggeredAt: at('2026-07-30', '11:20'), dueOverride: '2026-09-10', overrideReason: 'Large Scale Investigation: the LSI planning meeting on 6 Aug stands in place of an initial case conference and set a review of all strands for 10 Sep (Clydeshore LSI procedures)' },
    ],
    linkedProcessIds: [],
    viewsRecordIds: ['vw_whinbrae_archie', 'vw_whinbrae_zofia', 'vw_whinbrae_linda'],
    riskAssessmentIds: [],
    ...
    detail: {
      ...
      inquiry: {
        ...
        interAgencyDiscussionMeetingId: WHINBRAE.planningMeeting,
      ...
      investigation: {
        ...
        interviews: [
          ...
          { at: at('2026-08-12', '11:15'), power: 's8', withPersonId: margaret.id, note: 'Interviewed in private. Describes evenings when her tablets came "long after tea" and feeling stiff and unsteady the next morning. Wants to stay at Whinbrae House.' },
        ...
        recordsRequests: [
          { requestedAt: at('2026-08-07', '11:00'), power: 's10', holder: 'Whinbrae House', holderAgency: 'third-sector', status: 'received', note: 'MAR charts April to July, staff rota, incident log and the personal allowance ledger. Received 7 Aug.' },
          { requestedAt: at('2026-08-07', '11:30'), power: 's10', holder: 'Craiglarrick Health Centre', holderAgency: 'health', status: 'received', note: 'GP records for all six residents. Inspected by Dr Amira Farouk as the health professional, s10(4). Received 11 Aug.' },
        ],
        consent: {
  ...
  const invitees = (held: boolean): Invitee[] => {
    ...
    return [
      ...
      { name: HOME_MANAGER, agency: 'third-sector', role: 'Home manager, Whinbrae House', required: true, attendance: held ? 'present' : 'invited', reason: 'Provider: decisions and actions affecting the service only' },
      { name: PROVIDER_REP, agency: 'third-sector', role: 'Operations director, Whinbrae House Care (Scotland) Ltd', required: false, attendance: held ? 'present' : 'invited', reason: 'Provider representative' },
    ];
  };

  makeMeeting(ctx, {
    id: WHINBRAE.planningMeeting,
    type: 'lsi-planning',
    processId: process.id,
    subjectIds: all,
    title: 'LSI planning meeting: Whinbrae House',
    ...
    pack: [
      ...
      { id: 'pk_rb_plan_3', kind: 'views', label: 'Linda Paterson (carer) views, 5 Aug', ref: 'vw_whinbrae_linda', included: true },
    ...
    decisions: [
      ...
      { id: 'dec_rb_plan_5', question: 'Commissioning', decision: 'Pause new placements at Whinbrae House until the review meeting. Contract meeting with the provider by 28 Aug.', rationale: 'Precautionary while evening medication administration is unsafe; existing residents stay with the immediate safety measures in place.', dissent: [{ byName: PROVIDER_REP, agency: 'third-sector', text: 'The provider does not accept that a pause on placements is proportionate when the agency nurse concerned no longer works at the home and two nurses now cover every evening round. Asked that the pause be reviewed after four weeks rather than at the review meeting.' }], decidedByName: name(moira), decidedByUserId: moira, decidedAt: at('2026-08-06', '11:40') },
    ...
    viewsRecordIds: ['vw_whinbrae_linda'],
    minute: { status: 'distributed', draftedAt: at('2026-08-07', '15:00'), approvedAt: at('2026-08-10', '09:30'), distributedAt: at('2026-08-10', '10:00') },
    distribution: [
      { id: 'dist_rb_plan_1', recipientName: name(morven), recipientUserId: morven, agency: 'regulator', role: 'Inspector, Care Inspectorate', detailLevel: 'full', reason: 'Attendee; regulated service', sharingRecordId: 'shr_whinbrae_1' },
      { id: 'dist_rb_plan_2', recipientName: `Dr ${name(amira)}`, recipientUserId: amira, agency: 'health', role: 'GP', detailLevel: 'full', reason: 'Attendee; health input', sharingRecordId: 'shr_whinbrae_2' },
      { id: 'dist_rb_plan_3', recipientName: `DS ${name(paul)}`, recipientUserId: paul, agency: 'police', role: 'Detective sergeant', detailLevel: 'fields', fields: ['financial allegation', 'account and ledger details', 'relative contact', 'actions for police'], reason: 'Criminal element: the financial strand only', sharingRecordId: 'shr_whinbrae_3' },
      { id: 'dist_rb_plan_4', recipientName: name(alistair), recipientUserId: alistair, agency: 'regulator', role: 'OPG investigations officer', detailLevel: 'fields', fields: ['adult name and date of birth', 'nature of financial concern', 'whether a power of attorney or guardianship exists'], reason: 'Financial harm; attorney in place', sharingRecordId: 'shr_whinbrae_4' },
      { id: 'dist_rb_plan_5', recipientName: name(tam), recipientUserId: tam, agency: 'third-sector', role: 'Independent advocate', detailLevel: 'summary', reason: 'Advocacy offered', sharingRecordId: 'shr_whinbrae_5' },
      { id: 'dist_rb_plan_6', recipientName: name(andrew), recipientUserId: andrew, agency: 'social-work', role: 'Chief Social Work Officer', detailLevel: 'summary', reason: 'Oversight', sharingRecordId: 'shr_whinbrae_6' },
  ...
  makeMeeting(ctx, {
    id: WHINBRAE.reviewMeeting,
    type: 'lsi-planning',
    processId: process.id,
    subjectIds: all,
    title: 'LSI review of strands: Whinbrae House',
    ...
    pack: [
      ...
      { id: 'pk_rb_rev_4', kind: 'views', label: 'Archie Drummond (via advocate)', ref: 'vw_whinbrae_archie', included: true },
      { id: 'pk_rb_rev_5', kind: 'views', label: 'Zofia Wisniewska (via interpreter)', ref: 'vw_whinbrae_zofia', included: true },
      { id: 'pk_rb_rev_6', kind: 'report', label: 'Provider medication management action plan', included: true },
    ],
    actionIds: [...planningActionIds, 'act_whinbrae_9'],
    viewsRecordIds: ['vw_whinbrae_archie', 'vw_whinbrae_zofia'],
  ...
  share('shr_whinbrae_1', 'investigation', zofia.id, morven, name(morven), 'regulator', 'Inspector, Care Inspectorate', 'full', 'Regulated service: duty to cooperate under s5.', 'LSI planning minute, strands and actions; s7 visit and s10 records findings', at('2026-08-10', '10:00'), 'asp.conference.ci');
  share('shr_whinbrae_2', 'investigation', zofia.id, amira, `Dr ${name(amira)}`, 'health', 'GP', 'full', 'Health input needed. Records holder for the s10 request; a health professional inspects health records.', 'Medication error findings for all six residents; request to inspect GP records under s10 and lead the medication review', at('2026-08-07', '11:30'), 'asp.investigation.records');
  share('shr_whinbrae_3', 'investigation', wilma.id, paul, `DS ${name(paul)}`, 'police', 'Detective sergeant, PPU', 'fields', 'Criminal element: the financial strand only.', 'Allegation of five unexplained withdrawals from the personal allowance account; relative contact; ledger held by the provider', at('2026-08-06', '13:00'), 'asp.inquiry.police', ['financial allegation', 'account and ledger details', 'relative contact', 'actions for police']);
  share('shr_whinbrae_4', 'investigation', jean.id, alistair, name(alistair), 'regulator', 'OPG investigations officer', 'fields', 'Financial harm identified. A resident has a registered attorney.', 'Jean Kilgour, born 12 Mar 1938: register check requested; concern is about the service\'s personal allowance handling, not the attorney', at('2026-08-06', '13:10'), 'asp.investigation.opg', ['adult name and date of birth', 'nature of financial concern', 'whether a power of attorney or guardianship exists']);
  share('shr_whinbrae_5', 'investigation', archie.id, tam, name(tam), 'third-sector', 'Independent advocate', 'summary', 'Advocacy offered and accepted.', 'Archie Drummond lacks capacity for this decision; advocacy to support his views for the investigation and the review meeting', at('2026-08-07', '09:30'), 'asp.investigation.advocacy');
  share('shr_whinbrae_6', 'inquiry', jean.id, andrew, name(andrew), 'social-work', 'Chief Social Work Officer', 'summary', 'Large Scale Investigation opened: CSWO oversight.', 'LSI at Whinbrae House: six residents, medication errors and a financial allegation; planning meeting 6 Aug', at('2026-07-31', '12:00'));
  share('shr_whinbrae_7', 'investigation', margaret.id, undefined, `${HOME_MANAGER} (home manager)`, 'third-sector', 'Home manager, Whinbrae House', 'full', 'Provider asked for the interview records. Withheld: the provider is the subject of the investigation and receives decisions and actions only.', 'Request for s8 interview notes for Margaret Cairns withheld; provider sent the decisions and actions affecting the service', at('2026-08-14', '11:00'), 'asp.inquiry.provider', undefined, 'withheld');
  ...
  E({ occurredAt: at('2026-07-19', '11:40'), subjectIds: [zofia.id], agency: 'health', sourceSystem: 'trakcare', recordedByName: 'TrakCare connector', eventType: 'health.attendance', title: 'Emergency department: INR 5.8, bruising to both forearms', detail: 'Brought by ambulance from Whinbrae House after extensive bruising was noticed at breakfast. INR 5.8. Vitamin K given. Observed and returned to the home the same evening with a revised warfarin plan.', significance: 'high', significanceReason: 'Harm from a medication error' });
  ...
  E({ occurredAt: at('2026-08-06', '14:00'), subjectIds: all, agency: 'social-work', recordedByName: `${COMMISSIONING_LEAD} (contracts and commissioning)`, eventType: 'care.provider-concern', title: 'Commissioning paused new placements at Whinbrae House', detail: 'Clydeshore HSCP commissioning wrote to the provider confirming no new council-funded placements until the LSI review on 10 Sep. Existing residents unaffected.', significance: 'moderate' });
  ...
  E({ occurredAt: at('2026-08-11', '15:00'), subjectIds: [jean.id], agency: 'social-work', recordedByName: name(moira), recordedByUserId: moira, eventType: 'social-work.contact', title: 'Attorney consulted: Robert Kilgour agrees to the investigation and asks to be kept informed', detail: 'Telephone call. Robert Kilgour was not told about the omitted doses by the home. He agrees to the records check and to Jean staying at Whinbrae House while the evening staffing undertakings hold.', significance: 'moderate', linkedPersonIds: [robert.id] });
  ...
  E({ occurredAt: at('2026-08-12', '10:30'), subjectIds: [wilma.id], agency: 'social-work', recordedByName: name(stuart), recordedByUserId: stuart, eventType: 'voice.adult', title: 'Interviewed under s8: "never been told what is in the account"', detail: 'Wilma confirmed she did not ask for cash on the dates concerned. Wants her daughter to see the statements and to stay at Whinbrae House.', significance: 'moderate' });
  E({ occurredAt: at('2026-08-12', '11:15'), subjectIds: [margaret.id], agency: 'social-work', recordedByName: name(moira), recordedByUserId: moira, eventType: 'voice.adult', title: 'Interviewed under s8: tablets came "long after tea"', detail: 'Describes stiffness and unsteadiness the mornings after late doses. Wants to stay at Whinbrae House.', significance: 'moderate' });
  ...
  makeConnectorEvent(ctx, {
    ...
    sourcePayload: { patient: 'WISNIEWSKA, Zofia', practice: 'Craiglarrick Health Centre', clinician: 'Practice pharmacist', code: 'Medication review done', note: 'Warfarin: INR monitoring weekly for 4 weeks; home to use single-nurse witnessed administration; Polish medicines list provided' },
  ...
  makeConnectorEvent(ctx, {
    ...
    sourcePayload: { patient: 'RANKIN, Malcolm', practice: 'Craiglarrick Health Centre', clinician: 'Practice pharmacist', code: 'Medication review done', note: 'Insulin: pen device and timing confirmed; blood glucose diary to be kept by the home; no dose change' },
