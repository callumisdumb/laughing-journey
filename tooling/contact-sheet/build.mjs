...
const ORDER = ['phase-1', 'phase-2', 'phase-3', 'phase-4', 'phase-5', 'phase-6', 'classification', 'nmds', 'security', 'dialogs', 'layout', 'links', 'sign-in', 'clock', 'create', 'network', 'terminal', 'two-way', 'simulator', 'flows', 'search', 'demo', 'compare', 'recording', 'script', 'calendar'];
...
const ROUND_NOTES = {
  ...
  calendar: 'The working calendar behind every statutory clock: the national bank holiday list from the gov.uk feed with its provenance, the council\'s own local days kept separate from it, the next twelve months of non-working days, and a calculator that shows which days a count skipped and why.',
  script: 'The shooting script walked against the built product: each of the ten chapters of docs/DEMO.md opened from its own waypoint, at 1920x1080 with the recording preset on, showing the thing the script tells a viewer to look at.',
...
/* ---------------------------------------------- the handover's screenshot index, section 5 */

/*
 * Generated rather than maintained, because it was not maintained.
 *
 * Section 5 of the handover is the same list as this sheet without the pictures, and by the time
 * anybody looked it was missing eleven rounds: every capture round since the layout work had added
 * screenshots and nobody had added the rows. A list of files that has to be kept in step with the
 * files by hand is a list that goes out of date, so the generator that already reads the folder
 * writes it. Everything above and below the section is left exactly as it is.
 */
const indexBody = [
  '## 5. Screenshot index',
  '',
  `${total} screenshots under \`docs/SCREENSHOTS/<round>/<screen>-<theme>-<density>.png\`, captured by the Playwright suites at 1440 by 900 unless the round says otherwise (the \`compare\`, \`recording\` and \`script\` rounds are 1920 by 1080, which is what they are filmed at). Light comfortable is the default; dark and compact variants are listed where captured, and phase 6 holds the dark and compact sweep of every screen. **This section is generated by \`pnpm contact-sheet\`. Do not edit by hand.** \`docs/CONTACT-SHEET.md\` is the same list with the images, and \`docs/CONTACT-SHEET.html\` is a single self-contained file whose captures open at full size in a viewer; that one is gitignored.`,
  '',
];
for (const round of all) {
  indexBody.push(`### ${round.name}`, '');
  if (round.note) indexBody.push(round.note, '');
  indexBody.push('| Screen | Variants |', '|---|---|');
  const byScreen = new Map();
  for (const shot of round.shots) {
    const { screen, theme, density } = describe(shot.file);
    const variants = byScreen.get(screen) ?? [];
    variants.push(theme && density ? `${theme} ${density}` : 'as captured');
    byScreen.set(screen, variants);
  }
  for (const [screen, variants] of [...byScreen.entries()].sort((a, b) => a[0].localeCompare(b[0]))) {
    indexBody.push(`| ${screen} | ${[...new Set(variants)].sort().join(', ')} |`);
  }
  indexBody.push('');
}

const handoverPath = join(ROOT, 'docs', 'HANDOVER.md');
const handover = readFileSync(handoverPath, 'utf8');
const start = handover.indexOf('## 5. Screenshot index');
const end = handover.indexOf('## 6. ');
if (start === -1 || end === -1 || end < start) {
  console.error('contact sheet: could not find section 5 in docs/HANDOVER.md');
  process.exit(1);
}
writeFileSync(handoverPath, `${handover.slice(0, start)}${indexBody.join('\n').trimEnd()}\n\n${handover.slice(end)}`);
